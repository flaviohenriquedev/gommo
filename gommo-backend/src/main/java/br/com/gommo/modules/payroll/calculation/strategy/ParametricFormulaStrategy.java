...
import br.com.gommo.modules.payroll.calculation.domain.PayrollFormulaEvaluator;
...
public class ParametricFormulaStrategy implements PayrollCalculationStrategy {
    ...
    private static final Set<String> RESERVED_CODES = Set.of(
            "SAL_BASE", "HORA_EXTRA", "ADIC_NOTURNO", "INSALUBRIDADE", "PERICULOSIDADE", "INSS", "IRRF", "FGTS");

    @Override
    public void calculate(PayrollCalculationContext context) {
        for (PayrollEvent event : context.getEventsByCode().values()) {
            if (RESERVED_CODES.contains(event.getEventCode()) || context.getAppliedEventCodes().contains(event.getEventCode())) {
                continue;
            }
            if (event.getFormula() == null || event.getFormula().isBlank()) {
                continue;
            }
            if (event.getEventType() == PayrollEventTypeEnum.DEDUCTION || event.getEventType() == PayrollEventTypeEnum.INFORMATIVE) {
                continue;
            }
