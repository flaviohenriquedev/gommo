...
export function QueryPanel<T>({
    ...
    if (query.isError) {
        const error = query.error instanceof Error ? query.error : new Error("Erro ao carregar dados");
        return (
            errorFallback?.(error, () => void query.refetch()) ?? (
                <div className="m-5 rounded-box bg-error/10 p-5">
                    <p className="text-xs font-semibold text-error">{error.message}</p>
