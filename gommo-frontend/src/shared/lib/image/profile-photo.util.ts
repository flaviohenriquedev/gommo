...
export function baseCoverScale(naturalWidth: number, naturalHeight: number, viewportSize: number): number {
...
export async function exportProfilePhotoBlob(
    ...
    ctx.drawImage(image, area.x, area.y, area.width, area.height, 0, 0, outputMaxPx, outputMaxPx);
