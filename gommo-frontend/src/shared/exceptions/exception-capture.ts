...
/**
 * Captura e normaliza erros da API ou do cliente para {@link AppException}.
 * Use em CRUD, formulários e qualquer fluxo que precise exibir falhas ao usuário.
 */
export class ExceptionCapture {
  ...
  static fromUnknown(error: unknown, fallbackMessage?: string): AppException {
    if (error instanceof AppException) {
      return error.withDisplayMessage(
        resolveMessageByCode(error.code, error.displayMessage),
      );
    }
    ...
    if (error instanceof Error) {
      return AppException.client("UNKNOWN", error.message || fallbackMessage || AppException.unknown().displayMessage);
    }
