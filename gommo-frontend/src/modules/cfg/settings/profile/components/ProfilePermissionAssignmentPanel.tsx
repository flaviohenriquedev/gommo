...
import {type CheckboxState, InputCheckbox} from "@/shared/components/ui/input";

type ProfilePermissionAssignmentPanelProps = {
    system: SystemScope;
    onSystemChange: (system: SystemScope) => void;
    onSystemAccessToggle: (system: SystemScope) => void;
    systemAccessStateByScope: Partial<Record<SystemScope, CheckboxState>>;
    ...
    selectedPermissionCountByScope?: Partial<Record<SystemScope, number>>;
};

export function ProfilePermissionAssignmentPanel({
    system,
    onSystemChange,
    onSystemAccessToggle,
    systemAccessStateByScope,
    navSections,
    selectedRoute,
    markedRouteIds,
    onRouteSelect,
    ...
    selectedPermissionCountByScope,
}: ProfilePermissionAssignmentPanelProps) {
    return (
        <div className="grid min-h-[28rem] w-full grid-cols-1 overflow-hidden sm:col-span-12 lg:grid-cols-[minmax(20rem,24rem)_minmax(0,1fr)_minmax(14rem,18rem)]">
            <div className="flex min-h-0 min-w-0 flex-col border-b border-base-content/8 bg-base-200/25 lg:border-r lg:border-b-0">
                <div className="shrink-0 px-4 pt-3 pb-2">
                    ...
                    <p className="mt-0.5 text-[11px] text-base-content/45">
                        Marque o acesso completo por sistema
                    </p>
                </div>
                <ul className="flex min-h-0 flex-1 flex-col gap-2.5 overflow-y-auto px-3 pb-3">
                    {ASSIGNABLE_SYSTEM_SCOPES.map((scope) => {
                        ...
                        const selectedCount = active
                            ? selectedPermissionCount
                            : (selectedPermissionCountByScope?.[scope] ?? 0);
                        const accessState = systemAccessStateByScope[scope] ?? "unchecked";
                        return (
                            <li key={scope}>
                                <div
                                    className={clsx(
                                        "group flex w-full items-center gap-2.5 rounded-xl border px-3 py-3.5 transition-all duration-200",
                                        active
                                ...
                                >
                                    <InputCheckbox
                                        ...
                                        aria-label={
                                            accessState === "checked"
                                                ? `Remover acesso completo a ${infos.name}`
                                                : `Conceder acesso completo a ${infos.name}`
                                        }
                                        onChange={() => onSystemAccessToggle(scope)}
                                    />
                                    <button
                                        type="button"
                                        ...
                                        {selectedCount > 0 ? (
                                            ...
                                            <span
                                                className={clsx(
                                                    ...
                                                    active
                                                        ? "bg-primary"
                                                        : "bg-transparent group-hover:bg-base-content/20",
                                                )}
                                            />
                                        )}
                                    </button>
