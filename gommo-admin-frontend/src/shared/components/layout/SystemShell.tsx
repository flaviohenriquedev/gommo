...
export function SystemShell({ children }: { children: ReactNode }) {
    ...
    return (
        <Suspense
            fallback={
                <div className="flex h-dvh items-center justify-center text-sm text-base-content/50">Carregando…</div>
            }
        >
            <WorkspaceNavigationProvider>
                <div className="flex h-dvh flex-col overflow-hidden">
                    ...
                    />
                    <div className="flex h-dvh min-h-0 flex-1 flex-col overflow-hidden transition-[margin] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] lg:ms-(--sidebar-current,var(--sidebar-width))">
                        <header className="surface-header sticky top-0 z-30 flex h-(--header-height) shrink-0 items-center justify-between gap-3 px-4 sm:gap-4 sm:px-6">
                            {/* Mobile nav toggle */}
                            <button
                                type="button"
