...
import { usersFormPath, usersListPath } from "@/shared/routing/admin-nav";
...
export function AdminUsersModule() {
    ...
    useEffect(() => {
        ...
        void adminUserService
            ...
            .catch(() => {
                if (!cancelled) {
                    navigate(usersListPath(), { view: "list", recordId: "" }, "replace");
                }
            });
        return () => {
            cancelled = true;
    ...
    const openForm = (user: AdminUser) => {
        navigate(usersFormPath(user.id), { view: "form", recordId: user.id });
    };

    const onTabSelect = (key: string) => {
        if (key === "list") {
            navigate(usersListPath(), { view: "list", recordId: "" });
            return;
        }
        if (recordId && recordId !== "new") {
            navigate(usersFormPath(recordId), { view: "form", recordId });
            return;
        }
        if (selected) {
            navigate(usersFormPath(selected.id), { view: "form", recordId: selected.id });
            return;
        }
        navigate(usersFormPath("new"), { view: "form", recordId: "new" });
    };

    const handleSaved = async (user: AdminUser) => {
        await load();
        setSelected(user);
        if (recordId === "new" || recordId !== user.id) {
            navigate(usersFormPath(user.id), { view: "form", recordId: user.id }, "replace");
        }
    };
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
                {view === "list" ? (
                    <>
                        ...
                        <AdminDataGrid
                            cols={[
                                { key: "code", label: "Código", width: 80, dataType: DataType.INTEGER },
                                { key: "fullName", label: "Nome completo", dataType: DataType.STRING },
                            ...
                            onDoubleClick={(row) => openForm(row as unknown as AdminUser)}
